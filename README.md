...
There is no build step. Each `index.html` fetches its `.tex` file and renders it with `lib/hatex.js` (hatex 1.2.0). Each `tikz/` folder holds the pre-rendered TikZ figures (`<hash>.svg`), so readers never wait for TeX.
...
Released into the public domain under the [Unlicense](LICENSE). The classical Chinese texts quoted in `zh/` are themselves in the public domain.
