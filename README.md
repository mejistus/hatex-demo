...
There is no build step. Each `index.html` fetches its `.tex` file and renders it with `lib/hatex.js` (hatex 1.3.0). Each `tikz/` folder holds the pre-rendered TikZ figures (`<hash>.svg`), so readers never wait for TeX.
